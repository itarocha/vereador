function confirmdelInformatica (id) {
    jQuery.fancybox({
        closeBtn : false,
        closeClick : false,
        modal      : true,
        ...
        + '<input type="submit" class="f-left b-submit-maior c-branco bg-verde margin-auto b-ic-adicionar" value="OK" onclick="deletaInformatica(\'' + id + '\');" />'
...
function deletaInformatica (id) {
    $.post(LINK_MODULE + "gerenciamentovagas/ajaxdelinformatica", {
        "id": id
    }, function(data){
        if (data.msg == 'ok') {
            closeFancyboxAndRedirectToUrl(LINK_CONTROLLER + '/informatica');
        } else {
            fancyAlert('Erro de execução do processo. <br/>Gentileza tentar o procedimento novamente, caso ocorra novamente o problema contate a Ser Humano informando o ocorrido, obrigado!', 'err', LINK_CONTROLLER + '/informatica');
        }
    }, "json");
}
