function confirmdelCurso (id) {
    jQuery.fancybox({
        ...
        + '<input type="submit" class="f-left b-submit-maior c-branco bg-verde margin-auto b-ic-adicionar" value="OK" onclick="deletaCurso(\'' + id + '\');" />'
...
function deletaCurso (id) {
    $.post(LINK_MODULE + "gerenciamentovagas/ajaxdelcurso", {
        "id": id
    }, function(data){
        if (data.msg == 'ok') {
            closeFancyboxAndRedirectToUrl(LINK_CONTROLLER + '/cursoformacao');
        } else {
            fancyAlert('Erro de execução do processo. <br/>Gentileza tentar o procedimento novamente, caso ocorra novamente o problema contate a Ser Humano informando o ocorrido, obrigado!', 'err', LINK_CONTROLLER + '/cursoformacao');
        }
    }, "json");
}
